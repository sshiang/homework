...
public class QryEval {
  ...
  private static RetrievalModel initializeRetrievalModel (Map<String, String> parameters)
    ...
    if (modelString.equals("unrankedboolean")) {
	  ...
	  model = new RetrievalModelRankedBoolean();
		...
		String svmRankParamC = parameters.get("letor:svmRankParamC");
		...
		((RetrievalModelLetor) model).setParameters(trainingQrelsFile, trainingQueryFile, trainingFeatureVectorsFile, featureDisable, pageRankFile, svmRankLearnPath, svmRankClassifyPath, svmRankParamC, svmRankModelFile, testingFeatureVectorsFile, testingDocumentScores); 

		// getResult from bm25
		String k_1 = parameters.get("BM25:k_1");
		String b = parameters.get("BM25:b");
		String k_3 = parameters.get("BM25:k_3");
		RetrievalModelBM25 model_bm25 = new RetrievalModelBM25();
		((RetrievalModelBM25) model_bm25).setParameters(k_1, b, k_3);

		// getResult from indri
		String mu = parameters.get("Indri:mu");
		String lambda = parameters.get("Indri:lambda");
		RetrievalModelIndri model_indri = new RetrievalModelIndri();
		((RetrievalModelIndri) model_indri).setParameters(lambda, mu);

		((RetrievalModelLetor) model).setModelParameters(model_bm25, model_indri);
   ...
   */
  static ScoreList processQuery(String qString, String defaultField, RetrievalModel model)
    throws IOException {

    String defaultOp = model.defaultQrySopName ();
    qString = defaultOp + "(" + qString + ")";
    Qry q = QryParser.getQuery (qString, defaultField);
  ...
  static LinkedHashMap<Integer, LinkedHashMap<String, Double>> processSVMQuery(String qid, String qString, RetrievalModelLetor model, OutputStreamWriter output) throws Exception {

	LinkedHashMap<Integer, LinkedHashMap<String, Double>> feaMap = model.constructFeatureVector(qid, qString, "test");
	return feaMap;
  ...
  static void processQueryFile(String queryFilePath, String trecEvalOutputPath,
    ...
    try {
      ...
      //  Each pass of the loop processes one query.
		...
		// letor: svm rank training 
		OutputStreamWriter output_svm = null;
		if (model instanceof RetrievalModelLetor) {
			// train
			((RetrievalModelLetor)model).train();
		}
	  ...
	  HashMap<String, ScoreList> scoreListMap = readRankingList(initFileName);
		...
		//ArrayList<ArrayList<String>> qidLists = new ArrayList<ArrayList<String>>();

		LinkedHashMap<String, LinkedHashMap<Integer, LinkedHashMap<String, Double>>> testFeatureMap = new LinkedHashMap<String, LinkedHashMap<Integer, LinkedHashMap<String, Double>>>();

	if (model instanceof RetrievalModelLetor) {
		((RetrievalModelLetor)model).initializeMap();
	}
      ...
      while ((qLine = input.readLine()) != null) {
		...
		if ((model instanceof RetrievalModelIndri) && ( ((RetrievalModelIndri)model).getFb()==true) ) {
			...
			if (((RetrievalModelIndri)model).getFbInitialRankingFile().equals("")) {
				r = processQuery(query, "body", model);
			} else {
			...
			r = processQuery(new_query, "body", model);
		} else if (model instanceof RetrievalModelLetor) {
			// testing
			LinkedHashMap<Integer, LinkedHashMap<String, Double>> feaMap = ((RetrievalModelLetor)model).constructFeatureVector(qid, query, "test");
			...
			((RetrievalModelLetor)model).minMap.put(qid, minList); //p = this.featureVector.minMap;
			((RetrievalModelLetor)model).maxMap.put(qid, maxList); // = this.featureVector.maxMap;


			System.out.println(feaMap.size());
			testFeatureMap.put(qid, feaMap);
		...
		// dump result for only svm rank model ...
		if (model instanceof RetrievalModelLetor) {

			File fout_svm = new File(((RetrievalModelLetor)model).getTestingFeatureVectorsFile());
            FileOutputStream fos_svm = new FileOutputStream(fout_svm);
            output_svm = new OutputStreamWriter(fos_svm);
			// writeFeature file
			for (String qid: testFeatureMap.keySet()) {
				for (int docid : testFeatureMap.get(qid).keySet()) {
					for (String feaid: testFeatureMap.get(qid).get(docid).keySet()) {
						double score = testFeatureMap.get(qid).get(docid).get(feaid);
						double normalizedScore = 0.0;
						int feaid_int = Integer.parseInt(feaid); 
						double minval = ((RetrievalModelLetor)model).minMap.get(qid).get(feaid_int);
						double maxval = ((RetrievalModelLetor)model).maxMap.get(qid).get(feaid_int); 
						if ( maxval != minval) {
							normalizedScore = (score-minval)/(maxval-minval);
						}
						testFeatureMap.get(qid).get(docid).put(feaid, normalizedScore);
					}
					// write 
					((RetrievalModelLetor)model).writeFeatures(qid, docid, output_svm, 0, testFeatureMap.get(qid).get(docid));
				}
			}

			// model test
			((RetrievalModelLetor)model).test();
			// read prediction from rank svm
			ArrayList<Double> results = ((RetrievalModelLetor)model).readReuslt(); 			
			// output
			int count = 0;
			for (String qid: testFeatureMap.keySet() ) {
				ScoreList r = new ScoreList();
				for (int docid: testFeatureMap.get(qid).keySet() ) {
					r.add(docid, results.get(count));
					count += 1;
				}
				writeResults(qid, r, output);
			}
